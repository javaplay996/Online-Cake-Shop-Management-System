        ...
        <div v-if="showFlag">
            <el-form :inline="true" :model="searchForm" class="form-content">
                <el-row :gutter="20" class="slt" :style="{justifyContent:contents.searchBoxPosition=='1'?'flex-start':contents.searchBoxPosition=='2'?'center':'flex-end'}">
                     ...
                     <el-form-item :label="contents.inputTitle == 1 ? '店员名称' : ''">
                         <el-input prefix-icon="el-icon-search" v-model="searchForm.dianyuanName" placeholder="店员名称" clearable></el-input>
                     </el-form-item>
                         
                     <el-form-item :label="contents.inputTitle == 1 ? '店员类型' : ''">
                        <el-select v-model="searchForm.dianyuanTypes" placeholder="请选择店员类型">
                            <el-option label="=-请选择-=" value=""></el-option>
                            <el-option
                               v-for="(item,index) in dianyuanTypesSelectSearch"
                               v-bind:key="index"
                ...
                <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
                    <el-form-item>
                        ...
                        <a style="text-decoration:none" class="el-button el-button--success"
                           v-if="isAuth('dianyuan','导入导出')"
                           icon="el-icon-download"
                           href="http://localhost:8080/wangshangdangaodian/upload/dianyuanMuBan.xls"
                        >批量导入店员数据模板</a>
                        &nbsp;
                        <el-upload
                                v-if="isAuth('dianyuan','导入导出')"
                                ...
                                :action="getActionUrl"
                                :on-success="dianyuanUploadSuccess"
                                :on-error="dianyuanUploadError"
                        ...
                        &nbsp;
                        <!-- 导出excel -->
                        <download-excel v-if="isAuth('dianyuan','导入导出')" style="display: inline-block" class = "export-excel-wrapper" :data = "dataList" :fields = "json_fields" name = "dianyuan.xls">
                            <!-- 导出excel -->
                            <el-button
                                    type="success"
                                    icon="el-icon-download"
                ...
                <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
                    ...
                    <el-table-column width="300" :align="contents.tableAlign"
                        ...
                        <template slot-scope="scope">
                            ...
                            <el-button v-if="isAuth('dianyuan','查看')" type="success" icon="el-icon-tickets" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
                            <el-button v-if="isAuth('dianyuan','修改')" type="primary" icon="el-icon-edit" size="mini" @click="addOrUpdateHandler(scope.row.id)">修改</el-button>
                            <el-button v-if="isAuth('dianyuan','删除')" type="danger" icon="el-icon-delete" size="mini" @click="deleteHandler(scope.row.id)">删除</el-button>

                            <el-button v-if="isAuth('dianyuan','修改')" type="success" icon="el-icon-tickets" size="mini" @click="resetPassword(scope.row.id)">重置密码</el-button>
                        </template>
                    </el-table-column>
                </el-table>
                <el-pagination
        ...
        methods: {
            chartDialog() {
                ...
                let params = {
                    ...
                    thisTable : {//当前表
                        tableName :'dianyuan',//当前表表名,
                        sumColum : 'dianyuan_number', //求和字段
                        date : 'insert_time',//分组日期字段
                        // string : 'dianyuan_name',//分组字符串字段
                        // types : 'dianyuan_types',//分组下拉框字段
                    },
                    // joinTable : {//级联表（可以不存在）
            ...
            getDataList() {
                ...
                if (this.searchForm.dianyuanName!= '' && this.searchForm.dianyuanName!= undefined) {
                    params['dianyuanName'] = '%' + this.searchForm.dianyuanName + '%'
                }
                         
                if (this.searchForm.dianyuanTypes!= '' && this.searchForm.dianyuanTypes!= undefined) {
                    params['dianyuanTypes'] = this.searchForm.dianyuanTypes
                }
                        
                params['dianyuanDelete'] = 1// 逻辑删除字段 1 未删除 2 删除
                ...
                //填充下拉框选项
                this.$http({
                    url: "dictionary/page?dicCode=dianyuan_types&page=1&limit=100",
                    method: "get",
                }).then(({data}) => {
                    if(data && data.code === 0){
                        this.dianyuanTypesSelectSearch = data.data.list;
                    }
                });
            },
            //每页数
            ...
            resetPassword(id) {
                ...
                //重置密码
                this.$http({
                    url: "dianyuan/resetPassword?id=" + id,
                    method: "get",
                    // id:id
                }).then(({data}) => {
                    if(data && data.code === 0){
            ...
            // 导入功能上传文件成功后调用导入方法
            dianyuanUploadSuccess(data){
                let _this = this;
                _this.$http({
                    url: "dianyuan/batchInsert?fileName=" + data.file,
                    method: "get"
                }).then(({data}) => {
                    if(data && data.code === 0){
                        _this.$message({
