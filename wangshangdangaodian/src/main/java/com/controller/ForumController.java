...
, "usersId"});//把级联的数据添加到view中,并排除id和创建时间字段,当前表的级联注册表
            ...
            //级联表 店员
            //级联表
            DianyuanEntity dianyuan = dianyuanService.selectById(forum.getDianyuanId());
            if(dianyuan != null){
            BeanUtils.copyProperties( dianyuan , view ,new String[]{ "id", "createTime", "insertTime", "updateTime", "yonghuId"
, "dianyuanId"
, "usersId"});//把级联的数据添加到view中,并排除id和创建时间字段,当前表的级联注册表
            view.setDianyuanId(dianyuan.getId());
            }
            //级联表 管理员
            //管理员表做额外的处理
    ...
    */
    ...
    public R save(@RequestBody ForumEntity forum, HttpServletRequest request){
        ...
        if(false)
            ...
            forum.setDianyuanId(Integer.valueOf(String.valueOf(request.getSession().getAttribute("userId"))));
        ...
        Wrapper<ForumEntity> queryWrapper = new EntityWrapper<ForumEntity>()
            ...
            .eq("dianyuan_id", forum.getDianyuanId())
...
//                            forumEntity.setDianyuanId(Integer.valueOf(data.get(0)));   //店员 要改的
        ...
        ForumEntity forum = forumService.selectById(id);
            if(forum !=null){
                ...
                //级联表
                    DianyuanEntity dianyuan = dianyuanService.selectById(forum.getDianyuanId());
                if(dianyuan != null){
                    BeanUtils.copyProperties( dianyuan , view ,new String[]{ "id", "createDate"});//把级联的数据添加到view中,并排除id和创建时间字段
                    view.setDianyuanId(dianyuan.getId());
                }
                    UsersEntity users = usersService.selectById(forum.getUsersId());
                if(users != null){
